...
import json, datetime, os, codecs
...
class SignalSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for number in range(1, 400000):
            url = self.start_url + str(number)
            yield SplashRequest(url, self.parse, args={'wait': 2}, meta={"number": number})
    ...
    def parse(self, response):
        ...
        signal = {key: cleaned_fields[key] for key in ['signal', 'category', 'sub_category']}
...
class JsonWriterPipeline:
    ...
    def process_item(self, item, spider):
        ...
        if isinstance(item_data, list):
            ...
            line = json.dumps(item_data, ensure_ascii=False, indent=4) + "\n"
            file.write(line)
